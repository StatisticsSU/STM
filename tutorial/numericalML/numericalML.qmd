...
### 3. The likelihood function and maximum likelihood for the Poisson model
...
The interactive graph below lets you explore how different $\lambda$ values give a different fit to the data (the model fit is the orange line in the left graph, which ideally should match the histogram of the data). The graph to the right shows the log-likelihood function. The orange dot marks out the log-likelihood value for the $\lambda$ value chosen with the slider. By clicking the checkbox you will see the fit based on the ML estimate. Note that the Poisson model is a bad fit to this data when all 1000 auctions is used, even with the ML estimate. This is because the different auction have very different *reservation prices*. The reservation price of an auction is the lowest price that the seller will sell for. A high reservation price will then naturally discourage bids and will in general lead to fewer bidders in the auction. You can choose to only use the subset of the data with low reservation prices, or the ones with high reservation price, or all auctions. Instead of splitting the data the data according to some arbitrary threshold on the reservation price, we can use the reservation prices as a covariate/feature in a Poisson regression model.
:::

```{=html}
<iframe width="100%" height="608" frameborder="0"
  src="https://observablehq.com/embed/@mattiasvillani/maximum-likelihood-for-iid-poisson-data@2429?cells=viewof+settings%2Cviewof+dataselect%2Ctextslh%2Cbothplots"></iframe>
```
...
### 4. Derivation of the ML estimator and its standard error
...
### 5. Numerical optimization to find the ML estimator and its standard error
...
### 6. Approximate sampling distribution of the ML estimator in large samples
...
You can experiment with the five parameters in the bivariate normal distribution at this interactive widget. Note in particular what happens when you change the correlation coefficient parameter $\rho$ to positive and negative values.
:::

```{=html}
<iframe width="100%" height="777" frameborder="0"
  src="https://observablehq.com/embed/@mattiasvillani/multivariate-normal-distribution@364?cells=viewof+param%2Cplttrue"></iframe>
```
