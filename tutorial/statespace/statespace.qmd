...
plot(seq(1,n), y, type = "l", col = "steelblue", xlab = "time, t", ylab = "y_t", main = "Simulated data from the naive iid model")
lines(seq(1,n), rep(mu,n), type = "l", col = "orange")
legend("topright", legend = c(TeX("$y_t$"), TeX("$\\mu$")), lty = 1, 
...
<iframe width="100%" height="682" frameborder="0"
  src="https://observablehq.com/embed/@mattiasvillani/kalman-filtering-nile-river-data?cells=viewof+input_filt%2Cviewof+plotselector%2Cplotfilter"></iframe>
```
...
So we only need to set $\sigma_\varepsilon^2$ and $\sigma_\nu^2$ to start the fun. We will for now set $\sigma_\varepsilon^2 = 100^2$ and $\sigma_\nu^2 = 100^2$, and return to this when we learn how the dlm package can find maximum likelihood estimates for these parameters. Here is how you setup the local level model in the `dlm` package:
